...
class IMBALANCECIFAR10(torchvision.datasets.CIFAR10):
    ...
    def __init__(self, mode, cfg, root = './datasets/imbalance_cifar10', imb_type='exp',
        ...
        self.cfg = cfg
        self.train = train
        rand_number = cfg.IMBALANCECIFAR_RANDOM_SEED
        if self.train:
            np.random.seed(rand_number)
            random.seed(rand_number)
            imb_factor = self.cfg.IMBALANCECIFAR_RATIO
        ...
        if self.train and not self.cfg.SAMPLER_TYPE == "instance_balance":
    ...
    def __getitem__(self, index):
        ...
        """
        if self.train:
            assert self.cfg.SAMPLER_TYPE in ["instance_balance", "class_balance","reverse"]
